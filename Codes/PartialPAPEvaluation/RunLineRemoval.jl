...
# Enumerate feasible line removal moves
function enumerate_line_removal_moves(lineplan, network, demand, tp; silent_moves=true)
    moves = NamedTuple[]
    ...
    for L_a in 1:nlines
        ...
        lp_new = silent_moves ? redirect_stdout(devnull) do
            line_removal(lineplan, L_a)
        end : line_removal(lineplan, L_a)
        ...
        if feasible
            push!(moves, (L_a=L_a, lineplan_new=lp_new))
        end
    end
...
# Main sweep
function run_line_removal_sweep(; tp=5,
                                network_file="MandlNetwork.txt",
                                demand_file="MandlDemand.txt",
                                out_xlsx="LineRemoval_Sweep.xlsx",
                                silent_moves=true)
    ...
    moves = enumerate_line_removal_moves(lineplan, network, demand, tp; silent_moves=silent_moves)
    ...
    for m in moves
        ...
        A_rule = detect_line_removal(shortest_old, directlink_old, directlink_new,
                                     lineplan, m.L_a, tp;
                                     path_old=path_old,
        ...
        for r in 1:5
            push!(t_rule_runs, @elapsed begin
                detect_line_removal(shortest_old, directlink_old, directlink_new,
                                    lineplan, m.L_a, tp;
                                    path_old=path_old,
                                    lineinfo_old=lineinfo_old,
                                    lineinfo_new=lineinfo_new)
...
# Run
results = run_line_removal_sweep()