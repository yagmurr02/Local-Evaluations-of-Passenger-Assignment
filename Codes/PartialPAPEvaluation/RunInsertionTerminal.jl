...
# Enumerate feasible insertion at terminal moves
function enumerate_insertion_terminal_moves(lineplan, network, demand, tp;
                                            candidate_nodes = 1:size(network,1),
                                            positions=("start","end"),
                                            silent_moves=true)
        ...
        for pos in positions
            for x in candidate_nodes
                ...
                lp_new = silent_moves ? redirect_stdout(devnull) do
                    insertion_at_terminal(lineplan, line_id, x; position=pos)
                end : insertion_at_terminal(lineplan, line_id, x; position=pos)

                feasible, _, _ = is_feasible_candidate(lp_new, network, demand, tp)

                if feasible
                    push!(moves, (line_id=line_id,
                                  position=String(pos),
                                  x=x,
                                  lineplan_new=lp_new))
                end
...
# Main sweep
function run_insertion_terminal_sweep(; tp=5,
                                      network_file="MandlNetwork.txt",
                                      demand_file="MandlDemand.txt",
                                      out_xlsx="InsertionTerminal_Sweep.xlsx",
                                      silent_moves=true)
    ...
    moves = enumerate_insertion_terminal_moves(lineplan, network, demand, tp;
                                               candidate_nodes=1:size(network,1),
    ...
    for m in moves
        ...
        A_rule = detect_insertion_terminal(shortest_old,
                                           directlink_old,
                                           directlink_new,
                                           lineplan,
                                           m.line_id,
        ...
        for r in 1:5
            push!(t_rule_runs, @elapsed begin
                detect_insertion_terminal(shortest_old,
                                          directlink_old,
                                          directlink_new,
                                          lineplan,
                                          m.line_id,
        ...
        push!(results, (
            ...
            nodes_sub === nothing ? 0 : length(nodes_sub),
            bad,
            total,
...
# Run
results = run_insertion_terminal_sweep()