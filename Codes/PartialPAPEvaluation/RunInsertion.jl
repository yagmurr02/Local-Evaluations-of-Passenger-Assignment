...
# Enumerate feasible insertion moves
function enumerate_insertion_internal_moves(lineplan, network, demand, tp;
    ...
    for line_id in 1:nlines
        ...
        for i in 1:(length(stops)-1)
            ...
            for n_l in candidate_nodes
                ...
                if !(isfinite(network[n_j, n_l]) && network[n_j, n_l] > 0 &&
                     isfinite(network[n_l, n_k]) && network[n_l, n_k] > 0)
                    continue
                end

                lp_new = silent_moves ? redirect_stdout(devnull) do
                    insertion(lineplan, line_id, n_j, n_k, n_l, network)
                end : insertion(lineplan, line_id, n_j, n_k, n_l, network)
...
# Main sweep
function run_insertion_internal_sweep(; tp=5,
                                      network_file="MandlNetwork.txt",
                                      demand_file="MandlDemand.txt",
                                      out_xlsx="InsertionInternal_Sweep.xlsx",
                                      silent_moves=true)
    ...
    moves = enumerate_insertion_internal_moves(lineplan, network, demand, tp;
                                               candidate_nodes=1:size(network,1),
    ...
    for m in moves
        ...
        A_rule = detect_insertion_internal(shortest_old,
                                           directlink_old,
                                           directlink_new,
                                           lineplan,
                                           m.line_id,
        ...
        for r in 1:5
            push!(t_rule_runs, @elapsed begin
                detect_insertion_internal(shortest_old,
                                          directlink_old,
                                          directlink_new,
                                          lineplan,
                                          m.line_id,
        ...
        obj_gt = compute_objective_contribution(A_true, shortest_old, shortest_new, demand)
        obj_rule = compute_objective_contribution(A_rule, shortest_old, shortest_new, demand)
        ...
        if shortest_sub === nothing || nodes_sub === nothing
            ...
            bad, total = subset_quality(shortest_new, shortest_sub, nodes_sub, A_rule)
        end
...
# Run
results = run_insertion_internal_sweep()