...
# Simple path check: all stops must be unique (no cycles)
simple_path_ok(stops::Vector{Int}) = (length(unique(stops)) == length(stops))

# Enumerate feasible partial segment swap moves
function enumerate_partial_segment_swap_moves(lineplan, network, demand, tp;
                                              max_moves=40,
                                              rng_seed=1,
                                              silent_moves=true)
    ...
    line_pairs = [(L_a, L_b) for L_a in 1:nlines for L_b in (L_a+1):nlines]
    shuffle!(rng, line_pairs)

    for (L_a, L_b) in line_pairs
        length(moves) >= max_moves && break

        stops_a = Int.(filter(!=(0), lineplan[L_a, :]))
        length(stops_a) < 5 && continue

        stops_b = Int.(filter(!=(0), lineplan[L_b, :]))
        length(stops_b) < 5 && continue

        idx_pairs_a = [(i, j) for i in 2:(length(stops_a)-3) for j in (i+2):(length(stops_a)-1)]
        idx_pairs_b = [(p, q) for p in 2:(length(stops_b)-3) for q in (p+2):(length(stops_b)-1)]

        shuffle!(rng, idx_pairs_a)
        shuffle!(rng, idx_pairs_b)

        for (i, j) in idx_pairs_a
            length(moves) >= max_moves && break

            n_j = Int(stops_a[i])
            n_k = Int(stops_a[j])

            for (p, q) in idx_pairs_b
                length(moves) >= max_moves && break
                ...
                lp_new = silent_moves ? redirect_stdout(devnull) do
                    partial_segment_swap(lineplan, L_a, n_j, n_k, L_b, n_l, n_m, network)
                end : partial_segment_swap(lineplan, L_a, n_j, n_k, L_b, n_l, n_m, network)

                lp_new == lineplan && continue
    ...
    println("Enumerated partial-segment-swap moves: ", length(moves))
    return moves
end

# Main sweep
function run_partial_segment_swap_sweep(; tp=5,
                                        network_file="MandlNetwork.txt",
                                        demand_file="MandlDemand.txt",
                                        out_xlsx="PartialSegmentSwap_Sweep.xlsx",
                                        silent_moves=true,
                                        max_moves=40)
    ...
    moves = enumerate_partial_segment_swap_moves(lineplan, network, demand, tp;
                                                 max_moves=max_moves,
                                                 silent_moves=silent_moves)
    ...
    for m in moves
        ...
        A_rule = detect_partial_segment_swap(shortest_old, directlink_old, directlink_new,
                                             lineplan,
                                             m.L_a, m.n_j, m.n_k,
                                             m.L_b, m.n_l, m.n_m,
                                             tp,
        ...
        for r in 1:5
            push!(t_rule_runs, @elapsed begin
                detect_partial_segment_swap(shortest_old, directlink_old, directlink_new,
                                            lineplan,
                                            m.L_a, m.n_j, m.n_k,
                                            m.L_b, m.n_l, m.n_m,
                                            tp,
...
# Run
results = run_partial_segment_swap_sweep(max_moves=40)