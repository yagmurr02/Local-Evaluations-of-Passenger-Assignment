...
# Enumerate feasible swap within line moves
function enumerate_swap_within_line_moves(lineplan, network, demand, tp; silent_moves=true)
    moves = NamedTuple[]
    ...
    for line_id in 1:nlines
        ...
        for a in 1:length(stops)
            for b in (a+1):length(stops)
                n_j = Int(stops[a])
                n_k = Int(stops[b])

                lp_new = silent_moves ? redirect_stdout(devnull) do
                    swap_within_line(lineplan, line_id, n_j, n_k, network)
                end : swap_within_line(lineplan, line_id, n_j, n_k, network)
...
# Main sweep
function run_swap_within_line_sweep(; tp=5,
                                    network_file="MandlNetwork.txt",
                                    demand_file="MandlDemand.txt",
                                    out_xlsx="SwapWithinLine_Sweep.xlsx",
                                    silent_moves=true)
    ...
    moves = enumerate_swap_within_line_moves(lineplan, network, demand, tp; silent_moves=silent_moves)
    ...
    for m in moves
        ...
        A_rule = detect_swap_within_line(shortest_old,
                                         directlink_old,
                                         directlink_new,
                                         lineplan,
                                         m.line_id,
        ...
        for r in 1:5
            push!(t_rule_runs, @elapsed begin
                detect_swap_within_line(shortest_old,
                                        directlink_old,
                                        directlink_new,
                                        lineplan,
                                        m.line_id,
...
# Run
results = run_swap_within_line_sweep()