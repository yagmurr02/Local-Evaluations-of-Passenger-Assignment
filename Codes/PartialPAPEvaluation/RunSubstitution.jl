...
# Enumerate feasible substitution moves
function enumerate_substitution_moves(lineplan, network, demand, tp;
        ...
        for n_j in stops
            ...
            for n_k in candidate_nodes
                ...
                lp_new = silent_moves ? redirect_stdout(devnull) do
                    substitution(lineplan, line_id, n_j, n_k, network)
                end : substitution(lineplan, line_id, n_j, n_k, network)
...
# Main sweep
function run_substitution_sweep(; tp=5,
                                network_file="MandlNetwork.txt",
                                demand_file="MandlDemand.txt",
                                out_xlsx="Substitution_Sweep.xlsx",
                                silent_moves=true)
    ...
    moves = enumerate_substitution_moves(lineplan, network, demand, tp;
                                         candidate_nodes=1:size(network,1),
    ...
    for m in moves
        ...
        A_rule = detect_substitution(shortest_old,
                                     directlink_old,
                                     directlink_new,
                                     lineplan,
                                     m.line_id,
        ...
        for r in 1:5
            push!(t_rule_runs, @elapsed begin
                detect_substitution(shortest_old,
                                    directlink_old,
                                    directlink_new,
                                    lineplan,
                                    m.line_id,
...
# Run
results = run_substitution_sweep()