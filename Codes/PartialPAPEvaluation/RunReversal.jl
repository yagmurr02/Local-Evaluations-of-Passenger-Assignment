...
# Enumerate feasible reversal moves
function enumerate_reversal_moves(lineplan, network, demand, tp; silent_moves=true)
    moves = NamedTuple[]
    ...
    for line_id in 1:nlines
        ...
        lp_new = silent_moves ? redirect_stdout(devnull) do
            reversal(lineplan, line_id)
        end : reversal(lineplan, line_id)
...
# Main sweep
function run_reversal_sweep(; tp=5,
                            network_file="MandlNetwork.txt",
                            demand_file="MandlDemand.txt",
                            out_xlsx="Reversal_Sweep.xlsx",
                            silent_moves=true)
    ...
    moves = enumerate_reversal_moves(lineplan, network, demand, tp; silent_moves=silent_moves)
    ...
    for m in moves
        ...
        A_rule = detect_reversal(shortest_old,
                                 directlink_old,
                                 directlink_new,
                                 lineplan,
                                 m.line_id,
        ...
        if isfinite(obj_gt) && isfinite(obj_rule)
        ...
        if isfinite(obj_local_dijkstra) && isfinite(obj_full_eval)
            obj_gap_dijkstra = zero_if_tiny(obj_local_dijkstra - obj_full_eval)
        else
            obj_gap_dijkstra = BIG_M
        end

        if isfinite(obj_local_dijkstra_pq) && isfinite(obj_full_eval)
            obj_gap_dijkstra_pq = zero_if_tiny(obj_local_dijkstra_pq - obj_full_eval)
        else
            obj_gap_dijkstra_pq = BIG_M
        end

        if isfinite(obj_local_fw_subset) && isfinite(obj_full_eval)
            obj_gap_fw_subset = zero_if_tiny(obj_local_fw_subset - obj_full_eval)
        else
            obj_gap_fw_subset = BIG_M
        end
