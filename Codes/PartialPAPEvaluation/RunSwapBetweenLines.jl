...
# Enumerate feasible swap between lines moves
function enumerate_swap_between_lines_moves(lineplan, network, demand, tp; silent_moves=true)
    moves = NamedTuple[]
    nlines = size(lineplan, 1)

    for L_a in 1:nlines
        stops_a = Int.(filter(!=(0), lineplan[L_a, :]))
        isempty(stops_a) && continue

        for L_b in (L_a+1):nlines
            stops_b = Int.(filter(!=(0), lineplan[L_b, :]))
            isempty(stops_b) && continue
            ...
            for n_j in stops_a
                ...
                for n_k in stops_b
                    ...
                    lp_new = silent_moves ? redirect_stdout(devnull) do
                        swap_between_lines(lineplan, L_a, L_b, n_j, n_k, network)
                    end : swap_between_lines(lineplan, L_a, L_b, n_j, n_k, network)
...
# Main sweep
function run_swap_between_lines_sweep(; tp=5,
                                      network_file="MandlNetwork.txt",
                                      demand_file="MandlDemand.txt",
                                      out_xlsx="SwapBetweenLines_Sweep.xlsx",
                                      silent_moves=true)
    ...
    moves = enumerate_swap_between_lines_moves(lineplan, network, demand, tp; silent_moves=silent_moves)
    ...
    for m in moves
        ...
        A_rule = detect_swap_between_lines(shortest_old,
                                           directlink_old,
                                           directlink_new,
                                           lineplan,
                                           m.L_a,
        ...
        for r in 1:5
            push!(t_rule_runs, @elapsed begin
                detect_swap_between_lines(shortest_old,
                                          directlink_old,
                                          directlink_new,
                                          lineplan,
                                          m.L_a,
...
# Run
results = run_swap_between_lines_sweep()