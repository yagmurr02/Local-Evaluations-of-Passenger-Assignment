...
# Enumerate feasible transfer within line moves
function enumerate_transfer_within_line_moves(lineplan, network, demand, tp; silent_moves=true)
    moves = NamedTuple[]
        ...
        for i in 1:(length(stops)-1)
            ...
            for n_j in stops
                ...
                lp_new = silent_moves ? redirect_stdout(devnull) do
                    transfer_within_line(lineplan, line_id, n_j, n_k, n_l, network)
                end : transfer_within_line(lineplan, line_id, n_j, n_k, n_l, network)
...
# Main sweep
function run_transfer_within_line_sweep(; tp=5,
                                        network_file="MandlNetwork.txt",
                                        demand_file="MandlDemand.txt",
                                        out_xlsx="TransferWithinLine_Sweep.xlsx",
                                        silent_moves=true)
    ...
    moves = enumerate_transfer_within_line_moves(lineplan, network, demand, tp; silent_moves=silent_moves)
    ...
    for m in moves
        ...
        A_rule = detect_transfer_within_line(shortest_old,
                                             directlink_old,
                                             directlink_new,
                                             lineplan,
                                             m.line_id,
        ...
        for r in 1:5
            push!(t_rule_runs, @elapsed begin
                detect_transfer_within_line(shortest_old,
                                            directlink_old,
                                            directlink_new,
                                            lineplan,
                                            m.line_id,
...
# Run
results = run_transfer_within_line_sweep()