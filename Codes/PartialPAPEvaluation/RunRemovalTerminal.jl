...
# Objective settings
const GAP_TOL = 1.0e-9

# Helper: force tiny floating-point noise to zero
function zero_if_tiny(x::Float64; tol::Float64 = GAP_TOL)
...
# Extract the node removed by a terminal-removal move ("start" or "end")
function removed_terminal_node_local(lineplan, line_id::Int; position::String)
    stops = Int.(filter(!=(0), lineplan[line_id, :]))
    isempty(stops) && error("Line $line_id is empty.")
...
# Enumerate feasible removal at terminal moves
function enumerate_removal_terminal_moves(lineplan, network, demand, tp;
                                          positions=("start","end"),
                                          silent_moves=true)
    ...
    for line_id in 1:nlines
        ...
        for pos in positions
            lp_new = silent_moves ? redirect_stdout(devnull) do
                removal_at_terminal(lineplan, line_id; position=pos)
            end : removal_at_terminal(lineplan, line_id; position=pos)
            ...
            if feasible
                push!(moves, (
                    line_id=line_id,
                    position=String(pos),
                    removed_node=removed_terminal_node_local(lineplan, line_id; position=String(pos)),
                    lineplan_new=lp_new
                ))
            end
        end
...
# Main sweep
function run_removal_terminal_sweep(; tp=5,
                                    network_file="MandlNetwork.txt",
                                    demand_file="MandlDemand.txt",
                                    out_xlsx="RemovalTerminal_Sweep.xlsx",
                                    silent_moves=true)
    ...
    moves = enumerate_removal_terminal_moves(lineplan, network, demand, tp; silent_moves=silent_moves)
    ...
    for m in moves
        ...
        A_rule = detect_removal_terminal(shortest_old,
                                         directlink_old,
                                         directlink_new,
                                         lineplan,
                                         m.line_id,
        ...
        for r in 1:5
            push!(t_rule_runs, @elapsed begin
                detect_removal_terminal(shortest_old,
                                        directlink_old,
                                        directlink_new,
                                        lineplan,
                                        m.line_id,
        ...
        obj_gap = zero_if_tiny(obj_gt - obj_rule)
        ...
        obj_gap_dijkstra = zero_if_tiny(obj_local_dijkstra - obj_full_eval)
        obj_gap_dijkstra_pq = zero_if_tiny(obj_local_dijkstra_pq - obj_full_eval)
        obj_gap_fw_subset = zero_if_tiny(obj_local_fw_subset - obj_full_eval)
...
# Run
results = run_removal_terminal_sweep()