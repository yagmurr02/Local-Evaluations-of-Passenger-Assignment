...
# Enumerate feasible segment reversal moves
function enumerate_segment_reversal_moves(lineplan, network, demand, tp; silent_moves=true)
    moves = NamedTuple[]
        ...
        for a in 1:length(stops)
            for b in (a+1):length(stops)
                ...
                lp_new = silent_moves ? redirect_stdout(devnull) do
                    segment_reversal(lineplan, line_id, n_i, n_j)
                end : segment_reversal(lineplan, line_id, n_i, n_j)
...
# Main sweep
function run_segment_reversal_sweep(; tp=5,
                                    network_file="MandlNetwork.txt",
                                    demand_file="MandlDemand.txt",
                                    out_xlsx="SegmentReversal_Sweep.xlsx",
                                    silent_moves=true)
    ...
    moves = enumerate_segment_reversal_moves(lineplan, network, demand, tp; silent_moves=silent_moves)
    ...
    for m in moves
        ...
        A_rule = detect_segment_reversal(shortest_old,
                                         directlink_old,
                                         directlink_new,
                                         lineplan,
                                         m.line_id,
        ...
        for r in 1:5
            push!(t_rule_runs, @elapsed begin
                detect_segment_reversal(shortest_old,
                                        directlink_old,
                                        directlink_new,
                                        lineplan,
                                        m.line_id,
...
# Run
results = run_segment_reversal_sweep()