...
# Enumerate feasible transfer between lines moves
function enumerate_transfer_between_lines_moves(lineplan, network, demand, tp; silent_moves=true)
    moves = NamedTuple[]
            ...
            for i in 1:(length(stops_b)-1)
                ...
                for n_j in stops_a
                    ...
                    lp_new = silent_moves ? redirect_stdout(devnull) do
                        transfer_between_lines(lineplan, L_a, L_b, n_j, n_k, n_l, network)
                    end : transfer_between_lines(lineplan, L_a, L_b, n_j, n_k, n_l, network)
...
# Main sweep
function run_transfer_between_lines_sweep(; tp=5,
                                          network_file="MandlNetwork.txt",
                                          demand_file="MandlDemand.txt",
                                          out_xlsx="TransferBetweenLines_Sweep.xlsx",
                                          silent_moves=true)
    ...
    moves = enumerate_transfer_between_lines_moves(lineplan, network, demand, tp; silent_moves=silent_moves)
    ...
    for m in moves
        ...
        A_rule = detect_transfer_between_lines(shortest_old,
                                               directlink_old,
                                               directlink_new,
                                               lineplan,
                                               m.L_a,
        ...
        for r in 1:5
            push!(t_rule_runs, @elapsed begin
                detect_transfer_between_lines(shortest_old,
                                              directlink_old,
                                              directlink_new,
                                              lineplan,
                                              m.L_a,
...
# Run
results = run_transfer_between_lines_sweep()