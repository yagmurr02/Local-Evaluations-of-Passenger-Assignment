...
# Return all consecutive pairs of nodes in the route
...
function candidate_is_feasible(cand, network, demand, tp, n_nodes;
                               min_nodes::Int, max_nodes::Int)
    cand === nothing && return false

    return is_feasible(cand, network, demand, n_nodes;
                       tp = tp,
                       min_nodes = min_nodes,
                       max_nodes = max_nodes,
...
# Insertion
function random_insertion_move(lineplan, network, rng; min_nodes = 2, max_nodes = typemax(Int))

    n_routes = size(lineplan, 1)
    n_nodes  = size(network, 1)

    for L in randperm(rng, n_routes)
        route = vns_line_stops(lineplan, L)
...
function random_terminal_insertion_move(lineplan, network, rng; min_nodes = 2, max_nodes = typemax(Int))
    ...
    for L in randperm(rng, n_routes)
        ...
        for pos in positions
            for x in candidates
                ...
                if changed_lineplan(lineplan, cand)
                    move = VNSInsertionTerminal(L, x, pos)
...
# Removal
function random_removal_move(lineplan, network, rng; min_nodes = 2, max_nodes = typemax(Int))
...
# Removal at terminal
function random_terminal_removal_move(lineplan, network, rng; min_nodes = 2, max_nodes = typemax(Int))
    ...
    for L in randperm(rng, size(lineplan, 1))
        ...
        for pos in positions
            ...
            if changed_lineplan(lineplan, cand)
                ...
                move = VNSRemovalTerminal(L, removed_node, pos)
...
# Swap within line
function random_swap_within_line_move(lineplan, network, rng; min_nodes = 2, max_nodes = typemax(Int))
...
# Substitution
function random_substitution_move(lineplan, network, rng; min_nodes = 2, max_nodes = typemax(Int))
    ...
    for L in randperm(rng, size(lineplan, 1))
        ...
        for old in shuffle!(rng, copy(route))
            ...
            for new in candidates
                ...
                if changed_lineplan(lineplan, cand)
                    move = VNSSubstitution(L, old, new)
...
function random_partial_segment_swap_move(lineplan, network, rng; min_nodes = 2, max_nodes = typemax(Int))

    n_routes = size(lineplan, 1)
    n_routes < 2 && return nothing, nothing, "PartialSegmentSwap"

    line_pairs = [(i, j) for i in 1:(n_routes - 1) for j in (i + 1):n_routes]
    shuffle!(rng, line_pairs)

    for (La, Lb) in line_pairs
        ra = vns_line_stops(lineplan, La)
        rb = vns_line_stops(lineplan, Lb)
        ...
        for (aj, ak) in ea
            for (bl, bm) in eb
                ...
                if changed_lineplan(lineplan, cand)
                    move = VNSPartialSegmentSwap(La, aj, ak, Lb, bl, bm)
                    return cand, move, "PartialSegmentSwap"
                end
            end
        end
    end

    return nothing, nothing, "PartialSegmentSwap"
end

# Reversal
function random_reversal_move(lineplan, network, rng; min_nodes = 2, max_nodes = typemax(Int))
...
# Randomly select a neighborhood move type and generate a neighboring lineplan
function random_vns_neighbor(lineplan,
                             network,
                             demand,
                             tp,
                             n_nodes::Int,
    ...
    neighborhood_fns = [
        ...
        random_terminal_insertion_move,
        random_removal_move,
        random_terminal_removal_move,
        random_swap_within_line_move,
        random_substitution_move,
        random_partial_segment_swap_move,
        random_reversal_move
    ]

    for idx in randperm(rng, length(neighborhood_fns))
        cand, move, name = neighborhood_fns[idx](lineplan, network, rng;
                                                 min_nodes = min_nodes,
                                                 max_nodes = max_nodes)
        ...
        if candidate_is_feasible(cand, network, demand, tp, n_nodes;
                                 min_nodes = min_nodes,
                                 max_nodes = max_nodes)
...
# Generate a random feasible VNS neighboring lineplan
function generate_feasible_vns_neighbor(sol::TransitSolution,
                                        network,
                                        demand,
                                        tp,
                                        n_nodes::Int,
    ...
    for _ in 1:max_tries
        cand, move, name = random_vns_neighbor(sol.lineplan,
                                               network,
                                               demand,
                                               tp,
                                               n_nodes,
